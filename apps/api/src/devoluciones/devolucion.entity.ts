...
export class DevolucionActivo {
  ...
  @Column({ name: 'tiene_danios', default: false }) tieneDanios: boolean = false;
  @Column({ name: 'tiene_retraso', default: false }) tieneRetraso: boolean = false;
  @Column({ name: 'horas_retraso', type: 'numeric', precision: 8, scale: 2, default: 0 }) horasRetraso: number = 0;
...
export class DanoActivo {
  ...
  @Column({ type: 'jsonb', nullable: true }) fotos?: string[];
  @Column({ default: false }) resuelto: boolean = false;
  @CreateDateColumn({ name: 'created_at', type: 'timestamptz' }) createdAt!: Date;
}
